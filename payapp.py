...
.login-wrap { max-width: 460px; margin: 80px auto 0; padding: 40px; background: #fff; border: 1px solid #e5e1db; border-radius: 8px; border-top: 3px solid #d4521a; text-align: center; }
...
def get_allowed_emails():
        try:
                    return [e.strip().lower() for e in st.secrets["auth"]["allowed_emails"].split(",")]
except Exception:
        return ["vivek.naiwal@cars24.com"]

authenticator = Authenticate(
        secret_credentials_path=None,
        cookie_name="ct_transformer_auth",
        cookie_key=st.secrets.get("auth", {}).get("cookie_key", "ct_transformer_secret_key_2024"),
        redirect_uri=st.secrets.get("auth", {}).get("redirect_uri", "https://ct-data-transformer.streamlit.app"),
            ...
            <div class="login-wrap">
                    <div class="login-logo">🔐</div>
                            <div class="login-title">Payroll Portal</div>
                                    <div class="login-sub">CT Data Transformer — Restricted Access<br>Sign in with your Cars24 Google account</div>
                                        </div>
                                            """, unsafe_allow_html=True)
...
user_name = st.session_state.get("name", user_email)

allowed = get_allowed_emails()
if user_email not in allowed:
        st.error(f"❌ Access denied. Your account ({user_email}) is not authorised. Contact the administrator.")
...
if "pivot" in st.session_state:
    ...
    with dl1:
                st.download_button("⬇ Download Excel (.xlsx)", data=st.session_state["xlsx_bytes"],
                                                              file_name="employee_ct_pivoted.xlsx",
                                                              mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                                              use_container_width=True, key="dl_xlsx")
    with dl2:
                st.download_button("⬇ Download CSV (.csv)", data=st.session_state["csv_bytes"],
                                                              file_name="employee_ct_pivoted.csv", mime="text/csv",
    ...
    with st.expander("🔍 Diagnostics"):
        ...
        st.markdown(f"**Columns ({pivot.shape[1]}):** " + ", ".join(str(c) for c in list(pivot.columns)[:20]) +
